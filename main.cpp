...
int __cdecl main(void) 
{
	...
	if (input == "video") {
		...
		video >> image;
		if (image.empty()) {
		std::cout << "Input frame empty" << std::endl;
		return -1;
		}
		return 0;
		} else {
		image = cv::imread(input, 0);
		if (image.empty()) {
	...
	*/
	void camera_routine() {
		char cam_open[] = "VS_OPEN";
		sendCommand(cam_open, ClientSocket);
		char cam_queryframe[] = "VS_QUERYFRAME";
		sendCommand(cam_queryframe, ClientSocket);
		char cam_show[] = "VS_SHOW";
		sendCommand(cam_show, ClientSocket);
		char cam_save_dir[] = "VS_SAVE C:\DIR";
		sendCommand(cam_save_dir, ClientSocket);
		char stop_camera = "VS_CLOSE";
		sendCommand(stop_camera, ClientSocket);
	}
